...
import RepoLinksModal from './RepoLinksModal';

const Projects: React.FC = () => {
  const { projects } = portfolioData;
  const navigate = useNavigate();
  const [repoModalState, setRepoModalState] = useState<{
    isOpen: boolean;
    title: string;
    primaryRepo?: string;
  ...
  const openRepos = (project: Project) => {
    const hasExtraRepos = (project.links?.length ?? 0) > 0;
    ...
    setRepoModalState({
      isOpen: true,
      title: project.title,
      primaryRepo: project.github,
      links: project.links ?? []
    });
  };

  const closeRepos = () => {
    setRepoModalState(prev => ({ ...prev, isOpen: false }));
  };
  ...
  return (
    <section id="projects" className="py-20 bg-gray-50 dark:bg-black">
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="flex space-x-3">
                    ...
                    >
                      ...
                      <span>{(project.links?.length ?? 0) > 0 ? 'Repos' : 'GitHub'}</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="flex space-x-3">
                  ...
                  >
                    {(project.links?.length ?? 0) > 0 ? 'Repos' : 'GitHub'}
                  </button>
                  <button
                    onClick={() => {
                      navigate('/projects');
      ...
      <RepoLinksModal
        ...
        onClose={closeRepos}
        title={repoModalState.title}
        primaryRepo={repoModalState.primaryRepo}
        links={repoModalState.links}
      />
    </section>
  );
};
